...
| 2 | Multi-threaded; single-core serialization + divergence detection; retroactive watchpoints | **Done.** The recorder's single-core scheduler (`crates/recorder/src/capture/sched.rs`) serializes threads onto one core and records the interleaving as trace-format-v3 per-tid events (`SchedSwitch`/`ThreadSpawn`/`ThreadExit`). Replay now enforces that recorded schedule on the live tracee set (`crates/replay/src/mt.rs`): a `TidMap` remaps recorded tids to freshly cloned live pids, only the `SchedSwitch`-directed thread is ever resumed, and any mismatch surfaces as `ReplayError::ScheduleDiverged` rather than silently reordering. Fork-snapshot checkpointing auto-disables while more than one thread is live (a fork snapshot only captures the calling thread). Known limitation, follow-up work: a blocking syscall interrupted by the recorder's timer preemption leaves kernel-internal `-ERESTARTSYS` restart artifacts that replay does not yet reproduce, so target programs must avoid genuinely blocking syscalls under a live schedule. Retroactive hardware watchpoints (x86-64 debug registers, aarch64 `NT_ARM_HW_WATCH`) re-run a replay pass to report every access to an address across history, exposed via `mirrorscope watch` ([§13](#13-try-it)). |
| 3 | Move capture ptrace → eBPF (`aya`) tracepoints/uprobes | **In progress: capture observation only.** `aya`-based scaffolding is on `main` (`crates/recorder-ebpf*`): a raw-tracepoint `sys_enter`/`sys_exit` pair loads, attaches, and drains into the existing trace format, verified end-to-end against a BTF-enabled kernel. Behind the `ebpf` cargo feature via `record --ebpf`; the default build is unaffected. Gaps vs. the ptrace path, documented in the crates' module docs: no memory-payload capture (`data` is empty on every syscall-exit record), no serialization support (observation only, nothing to say about the single-core interleaving), and a single target tgid with no thread/process following. |
...
`stepBack`/`reverseContinue` are now checkpoint-accelerated: they restore the nearest fork-snapshot checkpoint instead of always re-executing from process entry, `listCheckpoints` returns the real snapshot list, and the interval is configurable via a `checkpointInterval` launch argument (default 200 trace-sequence units, `0` disables it).
...
Building with the `ebpf` cargo feature enables `mirrorscope record --ebpf -o trace.mscope -- ./my-program`, which captures via `aya` raw tracepoints instead of ptrace (observation only for now - see [§12](#12-status) for the gaps).
